...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $other_educational_qualification = trim($_POST['other_educational_qualification']);
    $professional_qualification = trim($_POST['professional_qualification']);
    $extra_curricular_activities = trim($_POST['extra_curricular_activities']);
    ...
    $ol_index_number = trim($_POST['ol_index_number']);
    $ol_year = $_POST['ol_year'];
    $ol_subjects = $_POST['ol_subjects'] ?? [];
    $ol_results = $_POST['ol_results'] ?? [];

    $al_index_number = trim($_POST['al_index_number']);
    ...
    try {
        // Insert into accountant table (position defaults to 'Accountant')
        $sql_accountant = "INSERT INTO accountant (username, password, full_name, gender, date_of_birth, postal_address, ethnicity, nic_number, marital_status, whatsapp_number, residence_number, first_language, profile_image, position) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Accountant')";
        $stmt_accountant = $conn->prepare($sql_accountant);
        $stmt_accountant->bind_param('sssssssssssss', $username, $password, $full_name, $gender, $date_of_birth, $postal_address, $ethnicity, $nic_number, $marital_status, $whatsapp_number, $residence_number, $first_language, $profile_image);
        $stmt_accountant->execute();

        // Insert into educational_details table
        $sql_educational_details = "INSERT INTO educational_details_accountant (username, other_educational_qualification, professional_qualification, extra_curricular_activities, work_experience) 
                                    VALUES (?, ?, ?, ?, ?)";
        ...
        // Insert past roles and job history
        if (!empty($previous_roles)) {
            $sql_previous_info = "INSERT INTO previous_info_accountant (username, previous_role, previous_company, years_experience) 
                                  VALUES (?, ?, ?, ?)";
            $stmt_previous_info = $conn->prepare($sql_previous_info);
            foreach ($previous_roles as $index => $role) {
                $stmt_previous_info->bind_param('sssi', $username, $previous_roles[$index], $previous_companies[$index], $years_experience[$index]);
                $stmt_previous_info->execute();
            }
        }

        // Insert OL results
        if (!empty($ol_subjects)) {
            $sql_ol_results = "INSERT INTO ol_result_accountant (username, subject_name, result, index_number, year) 
                               VALUES (?, ?, ?, ?, ?)";
            $stmt_ol_results = $conn->prepare($sql_ol_results);
            for ($i = 0; $i < count($ol_subjects); $i++) {
                $stmt_ol_results->bind_param('sssss', $username, $ol_subjects[$i], $ol_results[$i], $ol_index_number, $ol_year);
                $stmt_ol_results->execute();
            }
        }

        // Insert AL results
        if (!empty($al_subjects)) {
            $sql_al_results = "INSERT INTO al_result_accountant (username, subject_name, result, index_number, year) 
                               VALUES (?, ?, ?, ?, ?)";
            $stmt_al_results = $conn->prepare($sql_al_results);
            for ($i = 0; $i < count($al_subjects); $i++) {
                $stmt_al_results->bind_param('sssss', $username, $al_subjects[$i], $al_results[$i], $al_index_number, $al_year);
                $stmt_al_results->execute();
            }
        }
        ...
        $messageType = "success";
        echo '<meta http-equiv="refresh" content="2.5;url=add_accountant.php">';
    } catch (Exception $e) {
        $conn->rollback();
...
<body class="bg-gray-100 min-h-screen">
    ...
    <header class="bg-gradient-to-r from-yellow-900 to-yellow-700 text-white py-4 px-6 shadow-md ml-[68px] flex items-center">
        <div class="flex-1 text-center">
            <h1 class="text-2xl font-bold">Add New Accountant</h1>
        </div>
    </header>
    ...
    <main class="ml-[68px] p-6">
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 animate-bounceIn">
            <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Add New Accountant</h2>
            <form action="add_accountant.php" method="POST" enctype="multipart/form-data" id="accountantForm">
