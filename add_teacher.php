...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $subject_result = $conn->query($sql_subjects);
    ...
    $grade_result = $conn->query($sql_grades);
    ...
    // Dynamic grade fetching based on subject
    if (isset($_GET['subject_id']) && !empty($_GET['subject_id'])) {
        ...
        echo json_encode($grades); // Return grades dynamically for AJAX
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $grade_id = $_POST['class_teacher_grade'] ?? ''; 
    $teaching_classes = isset($_POST['grades']) && is_array($_POST['grades']) ? json_encode($_POST['grades']) : '';
    ...
    try {
        ...
        $sql_teacher = "
            INSERT INTO teacher (
                username, full_name, gender, date_of_birth, postal_address, ethnicity, nic_number,
                marital_status, whatsapp_number, residence_number, first_language, profile_image,
                subject_id, grade_id, teaching_classes
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        // 3. Insert into educational_details table
        $sql_educational_details = "
            INSERT INTO educational_details (username, other_educational_qualification, professional_qualification, extra_curricular_activities, work_experience) 
            VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql_educational_details);
        $stmt->bind_param('sssss', $username, $other_educational_qualification, $professional_qualification, $extra_curricular_activities, $work_experience);
        $stmt->execute();

        // 4. Insert past roles and job history
        foreach ($previous_roles as $index => $role) {
            if (!empty($role) && !empty($previous_companies[$index]) && !empty($years_experience[$index])) {
                $sql_previous_info = "
        ...
        for ($i = 0; $i < count($ol_subjects); $i++) {
            if (!empty($ol_subjects[$i]) && !empty($ol_results[$i])) {
                $sql_ol_results = "
                    INSERT INTO ol_result_teacher (username, subject_name, result, index_number, year) 
                    VALUES (?, ?, ?, ?, ?)";
        ...
        for ($i = 0; $i < count($al_subjects); $i++) {
            if (!empty($al_subjects[$i]) && !empty($al_results[$i])) {
                $sql_al_results = "
                    INSERT INTO al_result_teacher (username, subject_name, result, index_number, year) 
                    VALUES (?, ?, ?, ?, ?)";
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="ml-[68px] p-6">
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 animate-bounceIn">
            <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Add New Teacher</h2>
            <form action="" method="POST" enctype="multipart/form-data" id="teacherForm">
                ...
                <fieldset class="border border-gray-300 rounded-md p-6 mb-6 animate-bounceIn">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="form-group relative">
                            <select id="class_teacher_grade" name="class_teacher_grade" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 peer">
                                <option value="" disabled selected>Select Grade</option>
                                <?php foreach ($grades as $grade): ?>
                                    <option value="<?= $grade['id']; ?>"><?= htmlspecialchars($grade['grade_name']); ?></option>
                                <?php endforeach; ?>
                            </select>
                            <label for="class_teacher_grade" class="absolute left-3 top-3 text-sm font-medium text-gray-700 transition-all duration-200">Select Grade to be Class Teacher</label>
                        </div>
                        <div class="form-group relative">
                            <select id="subject" name="subject" class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 peer">
                                <option value="" disabled selected>Select Subject</option>
                                <?php foreach ($subjects as $subject): ?>
                                    <option value="<?= $subject['id']; ?>"><?= htmlspecialchars($subject['subject_name']); ?></option>
                                <?php endforeach; ?>
                            </select>
                            <label for="subject" class="absolute left-3 top-3 text-sm font-medium text-gray-700 transition-all duration-200">Select Subject</label>
                        </div>
                    </div>
                    <div id="grade_select" class="mt-4">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Select Grades</label>
                        <div id="grades" class="text-gray-600">Please select a subject to see grades.</div>
                    </div>
                </fieldset>
                        ...
                        .then(grades => {
                            ...
                            if (grades.length > 0) {
                                grades.forEach(grade => {
                                    ...
                                    gradeLabel.className = 'inline-flex items-center mr-4 mb-2';
                                    gradeLabel.innerHTML = `
                                        <input type="checkbox" name="grades[]" value="${grade.id}" class="mr-2">
                                        <span class="text-sm text-gray-700">${grade.grade_name}</span>
                                    `;
                                    gradeContainer.appendChild(gradeLabel);
                                });
                            } else {
                                gradeContainer.innerHTML = '<p class="text-gray-600">No grades available for this subject.</p>';
                            }
                        })
                        .catch(error => console.error('Error fetching grades:', error));
                } else {
                    gradeContainer.innerHTML = '<p class="text-gray-600">Please select a subject to see grades.</p>';
                }
            });
        });
