...
// Fetch previous roles from the database
$sql_previous_roles = "
    SELECT previous_role, previous_company, years_experience 
    FROM previous_info 
    WHERE username = ?";
$stmt_roles = $conn->prepare($sql_previous_roles);
$stmt_roles->bind_param("s", $username_filter);
$stmt_roles->execute();
$previous_roles_data = $stmt_roles->get_result()->fetch_all(MYSQLI_ASSOC);
...
// Decode teaching_classes to get the current grade IDs
$current_classes = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $assign_as_teacher = isset($_POST['assign_as_teacher']) ? true : false;
    ...
    // Insert or update job history
    $previous_roles = $_POST['previous_role'] ?? [];
    $previous_companies = $_POST['previous_company'] ?? [];
    $years_experience = $_POST['years_experience'] ?? [];

    $delete_roles = "DELETE FROM previous_info WHERE username = ?";
    $stmt_delete = $conn->prepare($delete_roles);
    $stmt_delete->bind_param("s", $username_filter);
    $stmt_delete->execute();

    foreach ($previous_roles as $index => $role) {
        $company = $previous_companies[$index] ?? '';
        $experience = $years_experience[$index] ?? '';

        $insert_role = "
            INSERT INTO previous_info (username, previous_role, previous_company, years_experience) 
            VALUES (?, ?, ?, ?)";
        $stmt_insert = $conn->prepare($insert_role);
        $stmt_insert->bind_param("sssi", $username_filter, $role, $company, $experience);
        $stmt_insert->execute();
    }

    // If the checkbox is checked, promote to teacher
    if ($assign_as_teacher && $grade_id) {
        $insert_teacher = "
            INSERT INTO teacher (username, full_name, gender, date_of_birth, postal_address, 
                nic_number, marital_status, whatsapp_number, residence_number, first_language, 
                profile_image, subject_id, grade_id, teaching_classes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $update_user_role = "UPDATE user SET role = 'Teacher' WHERE username = ?";
        $stmt_user = $conn->prepare($update_user_role);
        $stmt_user->bind_param("s", $username_filter);
        $stmt_user->execute();

        // Delete from `noclass_teacher`
        $delete_noclass_teacher = "DELETE FROM noclass_teacher WHERE username = ?";
        $stmt_delete_noclass = $conn->prepare($delete_noclass_teacher);
        $stmt_delete_noclass->bind_param("s", $username_filter);
        $stmt_delete_noclass->execute();
    }

    echo "
    <script>
        alert('Sub Teacher profile updated successfully.');
        window.location.href = 'sub_teacher_profile.php?username=" . urlencode($username_filter) . "';
    </script>
";
    exit;
}
...
<head>
    ...
    <title>Edit Substitute Teacher Profile</title>
...
<body class="bg-gray-100 min-h-screen">
        ...
        <div class="container max-w-5xl mx-auto bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 animate-bounceIn">
            <form method="POST" enctype="multipart/form-data" id="editProfileForm">
                ...
                <div class="profile-header bg-gradient-to-r from-yellow-900 to-yellow-700 text-white p-6 rounded-t-lg flex flex-col sm:flex-row items-center gap-4">
                    <div class="profile-img-container relative">
                        <?php if (!empty($teacher_details['profile_image']) && file_exists($teacher_details['profile_image'])): ?>
                            <img id="profileImagePreview" src="<?php echo htmlspecialchars($teacher_details['profile_image']); ?>" alt="Profile Image of <?php echo htmlspecialchars($teacher_details['full_name']); ?>" class="profile-img w-40 h-40 object-cover rounded-full transition-transform duration-300">
                        <?php else: ?>
                            <img id="profileImagePreview" src="./Resources/default_profile.png" alt="Default Profile Image" class="profile-img w-40 h-40 object-cover rounded-full transition-transform duration-300">
                        <?php endif; ?>
                    </div>
                    <div class="text-center sm:text-left">
                        <h2 class="text-2xl font-bold">Edit <?php echo htmlspecialchars($teacher_details['full_name']); ?>'s Profile</h2>
                        <p class="text-sm">Update substitute teacher details below</p>
                    </div>
                </div>
                ...
                <div class="accordion mt-6">
                    ...
                    <!-- Past Roles -->
                    <div class="accordion-item mb-4">
                        <button type="button" class="accordion-header w-full bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200 text-left text-lg font-semibold text-yellow-700 flex items-center" aria-expanded="false" aria-controls="roles-content">
                            <i class='bx bx-briefcase mr-2 text-yellow-600'></i> Past Roles and Job History
                            <i class='bx bx-chevron-down ml-auto text-yellow-600'></i>
                        </button>
                        <div id="roles-content" class="accordion-content grid grid-cols-1 gap-4 p-4">
                            <div class="card bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200">
                                <h3 class="text-sm font-medium text-gray-700 mb-2">Previous Roles</h3>
                                <div id="previous_roles_container" class="space-y-4">
                                    <?php if (!empty($previous_roles_data)): ?>
                                        <?php foreach ($previous_roles_data as $index => $role_data): ?>
                                            <div class="previous_role_entry bg-white p-4 rounded-md shadow hover:shadow-md transition duration-200">
                                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                                    <div>
                                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-1">
                                                            <i class='bx bx-briefcase mr-2 text-yellow-600'></i> Previous Role
                                                        </label>
                                                        <input type="text" name="previous_role[]" value="<?php echo htmlspecialchars($role_data['previous_role']); ?>" 
                                                               placeholder="e.g., Teacher, Administrator" required 
                                                               class="p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 w-full" aria-label="Previous Role">
                                                    </div>
                                                    <div>
                                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-1">
                                                            <i class='bx bx-building mr-2 text-yellow-600'></i> Previous Company/Organization
                                                        </label>
                                                        <input type="text" name="previous_company[]" value="<?php echo htmlspecialchars($role_data['previous_company']); ?>" 
                                                               placeholder="e.g., ABC School, XYZ Company" required 
                                                               class="p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 w-full" aria-label="Previous Company">
                                                    </div>
                                                    <div>
                                                        <label class="flex items-center text-sm font-medium text-gray-700 mb-1">
                                                            <i class='bx bx-time mr-2 text-yellow-600'></i> Years of Experience
                                                        </label>
                                                        <div class="flex items-center gap-2">
                                                            <input type="number" name="years_experience[]" value="<?php echo htmlspecialchars($role_data['years_experience']); ?>" 
                                                                   placeholder="e.g., 5" required min="0" 
                                                                   class="p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 w-full" aria-label="Years of Experience">
                                                            <button type="button" onclick="removeRole(this)" class="p-2 bg-red-600 text-white rounded-md btn-gradient hover:bg-red-700 transition duration-200" aria-label="Remove role">
                                ...
                                <button type="button" onclick="addMoreRoles()" class="mt-4 p-2 bg-gradient-to-r from-yellow-900 to-yellow-700 text-white rounded-md btn-gradient hover:bg-yellow-800 transition duration-200 hover-loop" aria-label="Add more roles">
                                    <i class='bx bx-plus mr-1'></i> Add More
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="accordion-item mb-4">
                        ...
                        <div id="teaching-content" class="accordion-content grid grid-cols-1 gap-4 p-4">
                            ...
                            <div class="card bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200">
                                <label class="flex items-center text-sm font-medium text-gray-700 mb-2">
                                    <i class='bx bx-user-check mr-2 text-yellow-600'></i> Assign as Class Teacher
                                </label>
                                <label class="flex items-center text-sm text-gray-600 custom-checkbox">
                                    <input type="checkbox" name="assign_as_teacher" id="assign_as_teacher" <?php echo isset($_POST['assign_as_teacher']) || $teacher_details['grade_id'] ? 'checked' : ''; ?> 
                                           class="mr-2">
                                    Assign as Class Teacher
                                </label>
                                <div id="teacher_grade_section" style="<?php echo isset($_POST['assign_as_teacher']) || $teacher_details['grade_id'] ? 'display:block;' : 'display:none;'; ?>">
                                    <label class="flex items-center text-sm font-medium text-gray-700 mt-2 mb-1">
                                        <i class='bx bx-book mr-2 text-yellow-600'></i> Select Grade for Class Teacher
                                    </label>
                                    <select name="grade_id" class="p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 w-full" aria-label="Class Teacher Grade">
                                        <option value="">Select Grade</option>
                                        <?php foreach ($grades as $grade): ?>
                                            <option value="<?php echo $grade['id']; ?>" <?php echo (isset($grade_id) && $grade_id == $grade['id']) || $teacher_details['grade_id'] == $grade['id'] ? 'selected' : ''; ?>>
                                                <?php echo htmlspecialchars($grade['grade_name']); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                ...
                <div class="action-bar no-print fixed bottom-4 right-4 z-40 flex flex-col sm:flex-row gap-2">
                    ...
                    <a href="./sub_teacher_profile.php?username=<?php echo urlencode($teacher_details['username']); ?>" 
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function previewImage(event) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        preview.classList.add('w-40', 'h-40', 'object-cover', 'rounded-full', 'transition-transform', 'duration-300', 'profile-img');
                    };
            ...
            document.getElementById('assign_as_teacher').addEventListener('change', function(event) {
                event.preventDefault(); // Prevent form submission
                document.getElementById('teacher_grade_section').style.display = this.checked ? 'block' : 'none';
            });
            ...
            function addMoreRoles() {
                ...
                newEntry.classList.add('previous_role_entry', 'bg-white', 'p-4', 'rounded-md', 'shadow', 'hover:shadow-md', 'transition', 'duration-200');
                newEntry.innerHTML = `
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="flex items-center text-sm font-medium text-gray-700 mb-1">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <input type="number" name="years_experience[]" placeholder="e.g., 5" required min="0" 
                                       class="p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-yellow-600 focus:border-yellow-600 transition duration-200 w-full" aria-label="Years of Experience">
                                <button type="button" onclick="removeRole(this)" class="p-2 bg-red-600 text-white rounded-md btn-gradient hover:bg-red-700 transition duration-200" aria-label="Remove role">
                                    <i class='bx bx-trash'></i>
                                </button>
            ...
            function removeRole(button) {
                button.closest('.previous_role_entry').remove();
            }
        });
    </script>
</body>
