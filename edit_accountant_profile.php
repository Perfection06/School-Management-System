...
$query = "
    SELECT accountant.*, educational_details_accountant.other_educational_qualification, 
           educational_details_accountant.professional_qualification, educational_details_accountant.extra_curricular_activities, 
           educational_details_accountant.work_experience
    FROM accountant 
...
$previous_roles_query = "SELECT * FROM previous_info_accountant WHERE username = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_accountant_query = "
        ...
        WHERE username = ?";
    $accountant_stmt = $conn->prepare($update_accountant_query);
    $accountant_stmt->bind_param(
        "sssssssssssss",
        $full_name,
        $gender,
        $dob,
    ...
    $update_edu_query = "
        UPDATE educational_details_accountant 
    ...
    $delete_roles_query = "DELETE FROM previous_info_accountant WHERE username = ?";
    ...
    $insert_role_query = "INSERT INTO previous_info_accountant (username, previous_role, previous_company, years_experience) VALUES (?, ?, ?, ?)";
    ...
    if ($accountant_stmt->execute() && $edu_stmt->execute()) {
        echo "
            <script>
                alert('Accountant profile updated successfully.');
                window.location.href = 'accountant_profile.php?username=" . urlencode($username_filter) . "';
            </script>
        ";
        exit;
    } else {
        echo "
            <script>
                alert('Error updating accountant profile. Please try again.');
                window.history.back();
            </script>
        ";
    }
...
<body>
    ...
    <!-- Back link -->
    <div class="back-link">
        <a href="accountant_profile.php?username=<?php echo urlencode($username_filter); ?>">Back to Profile</a>
    </div>
