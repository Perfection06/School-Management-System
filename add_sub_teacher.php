...
if (isset($_GET['subject_id']) && !empty($_GET['subject_id'])) {
    ...
    // Query to fetch grades linked to the selected subject
    $sql_grades = "
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Role and Credentials (Subject and Grades)
    $subject_id = $_POST['subject'];
    $grades = $_POST['grades']; // Array of selected grades
    $teaching_classes = json_encode($grades);
    ...
    try {
        ...
        $stmt = $conn->prepare($sql_noclass_teacher);
        $stmt->bind_param(
            'ssssssssssssis', 
            $username, 
            $full_name, 
        ...
        for ($i = 0; $i < count($ol_subjects); $i++) {
            $sql_ol_results = "INSERT INTO ol_result_teacher (username, subject_name, result, index_number, year) VALUES (?, ?, ?, ?, ?)";
        ...
        for ($i = 0; $i < count($al_subjects); $i++) {
            $sql_al_results = "INSERT INTO al_result_teacher (username, subject_name, result, index_number, year) VALUES (?, ?, ?, ?, ?)";
        ...
        $conn->commit();
        echo "<script>
        alert('Sub Teacher successfully added!');
...
<head>
    ...
    <title>Add New Sub Teacher</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="ml-[68px] p-6">
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 animate-bounceIn">
            <h2 class="text-2xl font-bold text-center text-gray-800 mb-6">Add New Sub Teacher</h2>
            <form action="" method="POST" enctype="multipart/form-data" id="subTeacherForm">
