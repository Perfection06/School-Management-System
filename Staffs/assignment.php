...
// Fetch the staff's username from the session
...
function deleteExpiredAssignments($pdo, $username) {
    ...
    // Fetch assignments for the logged-in staff
...
// Fetch all available grades to display in the dropdown
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload'])) {
    ...
    $selected_class_id = $_POST['class_id']; // Get the selected grade ID
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedExtensions) && $fileSize <= $maxFileSize) {
            ...
            if (move_uploaded_file($fileTmpPath, $filePath)) {
                // Insert the assignment into the database for the selected grade
                $stmt = $pdo->prepare("
                ...
                $stmt->execute([
                    ...
                    'class_id' => $selected_class_id // Store assignment for the selected grade
                ]);
...
// Fetch all assignments for the logged-in staff
$stmt = $pdo->prepare("SELECT * FROM assignments WHERE username = :username");
$stmt->execute(['username' => $username]);
$assignments = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<div class="container">
    ...
    <form action="assignment.php" method="POST" enctype="multipart/form-data">
        ...
        <!-- Dropdown for the grade -->
        <label for="class_id">Select Grade:</label>
