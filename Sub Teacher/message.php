...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($content === '') {
        echo "<script>alert('Message content cannot be empty.'); window.history.back();</script>";
        exit;
    }
    ...
    if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_type, $allowed_types)) {
            ...
            echo "<script>alert('Invalid file type. Allowed types: JPG, PNG, PDF, DOC, DOCX.'); window.history.back();</script>";
            exit;
        }
    }

    // Set receiver_username to 'admin'
    $receiver_username = 'admin';

    $stmt = $conn->prepare("INSERT INTO user_messages (sender_username, receiver_username, content, attachment_path, target_role) VALUES (?, ?, ?, ?, ?)");
    if ($stmt) {
        $stmt->bind_param('sssss', $username, $receiver_username, $content, $attachment_path, $role);
        if ($stmt->execute()) {
            echo "<script>alert('Message sent successfully!'); window.location.href = 'message.php';</script>";
        } else {
            echo "<script>alert('Failed to send message. Error: " . $stmt->error . "'); window.history.back();</script>";
        }
        $stmt->close();
    } else {
