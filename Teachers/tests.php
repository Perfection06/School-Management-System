...
$teacherUsername = $_SESSION['user']['username']; // Assuming the teacher's username is stored in session

// Fetch teaching_classes (JSON) and subject_id from the teacher's table
$query = "SELECT teaching_classes, subject_id FROM teacher WHERE username = ?";
...
if (empty($teachingClasses)) {
    die("No grades assigned to this teacher.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($type) || empty($gradeId) || empty($testDate) || empty($publishDate)) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO tests (type, grade_id, subject_id, teacher_username, test_date, publish_date)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        if ($stmt->execute([$type, $gradeId, $subjectId, $teacherUsername, $testDate, $publishDate])) {
            echo "<script>
