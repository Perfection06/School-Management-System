...
// Fetch grade names for grade_id
$teaching_classes = [];
if (!empty($teaching_classes_ids)) {
    ...
    $types = str_repeat('i', count($teaching_classes_ids));
    $stmt->bind_param($types, ...$teaching_classes_ids);
...
$stmt->bind_param("s", $username_filter);
$stmt->execute();
$ol_results = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
...
$stmt->bind_param("s", $username_filter);
$stmt->execute();
$al_results = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
...
$stmt->bind_param("s", $username_filter);
$stmt->execute();
$previous_info = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="ml-[68px] p-6">
        ...
        <div class="container max-w-5xl mx-auto bg-white p-8 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 animate-bounceIn">
            <?php if ($user_details): ?>
                ...
                <div class="profile-header bg-gradient-to-r from-yellow-900 to-yellow-700 text-white p-6 rounded-t-lg flex flex-col sm:flex-row items-center gap-4">
                    <div class="profile-img-container relative">
                        ...
                        <?php else: ?>
                            <div class="w-40 h-40 bg-gray-200 rounded-full flex items-center justify-center text-gray-500 text-sm">No Image</div>
                        <?php endif; ?>
                    ...
                    <div class="text-center sm:text-left">
                        ...
                        <p class="text-sm"><?php echo htmlspecialchars($user_details['grade_name'] ?? 'N/A'); ?> | <?php echo htmlspecialchars($user_details['subject_name'] ?? 'N/A'); ?></p>
                    </div>
                </div>
                ...
                <div class="action-bar no-print sticky top-0 z-40 bg-gray-50 p-4 rounded-b-lg shadow-md flex flex-wrap justify-center gap-2 mb-6">
                    ...
                    <a href="./edit_teacher_profile.php?username=<?php echo urlencode($username_filter); ?>" 
                ...
                <div class="tab-content">
                    ...
                    <div id="teaching" class="tab-pane hidden mb-6 animate-bounceIn">
                        <div class="card bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200 mb-4">
                            <div class="flex items-start">
                                <i class='bx bx-book mr-2 text-yellow-600 text-lg'></i>
                                <div>
                                    <label class="text-sm font-medium text-gray-700">Assigned Class:</label>
                                    <span class="block text-sm text-gray-600"><?php echo htmlspecialchars($user_details['grade_name'] ?? 'N/A'); ?></span>
                                </div>
                            </div>
                        </div>
                        <div class="card bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200 mb-4">
                        ...
                        <div class="card bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200 mb-4">
                            <div class="flex items-start">
                                <i class='bx bx-star mr-2 text-yellow-600 text-lg'></i>
                                <div>
                                    <label class="text-sm font-medium text-gray-700">Rank:</label>
                                    <span class="block text-sm text-gray-600"><?php echo htmlspecialchars($user_details['rank'] ?? 'N/A'); ?></span>
                                </div>
                            </div>
                        </div>
                        <div class="card bg-gray-50 p-4 rounded-md shadow hover:shadow-lg transition duration-200">
            ...
            <?php else: ?>
                <p class="text-center text-gray-500 italic animate-bounceIn">User not found.</p>
            <?php endif; ?>
        </div>
