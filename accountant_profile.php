...
$query = "
    SELECT accountant.*, user.*, educational_details_accountant.other_educational_qualification, educational_details_accountant.professional_qualification,
           educational_details_accountant.extra_curricular_activities, educational_details_accountant.work_experience, accountant.position, accountant.active
    FROM accountant 
    LEFT JOIN user ON accountant.username = user.username
    LEFT JOIN educational_details_accountant ON accountant.username = educational_details_accountant.username
    WHERE accountant.username = ?";
...
$experience_query = "SELECT * FROM previous_info_accountant WHERE username = ?";
...
$ol_query = "SELECT * FROM ol_result_accountant WHERE username = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['block']) && !empty($_POST['username']) && !empty($_POST['block_reason'])) {
        ...
        try {
            // Update accountant table to set active = 0 (block the accountant)
            $sql_block = "UPDATE accountant SET active = 0 WHERE username = ?";
            ...
            $sql_reason = "INSERT INTO accountant_block_reasons (username, block_reason) VALUES (?, ?)";
            ...
            $conn->commit();
            echo "<script>
                    alert('Accountant has been blocked successfully.');
                    window.location.href = 'view_teachers_staff.php'; // Redirect to a page showing accountants list
                  </script>";
        } catch (Exception $e) {
            $conn->rollback();
            echo "<p>Error blocking accountant: " . $e->getMessage() . "</p>";
        }
    }
    ...
    if (isset($_POST['activate']) && !empty($_POST['username'])) {
        ...
        try {
            // Update accountant table to set active = 1 (activate the accountant)
            $sql_activate = "UPDATE accountant SET active = 1 WHERE username = ?";
            ...
            $sql_delete_reason = "DELETE FROM accountant_block_reasons WHERE username = ?";
            ...
            $conn->commit();
            echo "<script>
                    alert('Accountant has been unblocked successfully.');
                    window.location.href = 'view_teachers_staff.php'; // Redirect to a page showing accountants list
                  </script>";
        } catch (Exception $e) {
            $conn->rollback();
            echo "<p>Error activating accountant: " . $e->getMessage() . "</p>";
        }
    }
}
...
<body>
    ...
    <a href="./edit_accountant_profile.php?username=<?php echo urlencode($username_filter); ?>">Edit</a>
    ...
    <?php if ($user_details['active'] == 1): ?>
        <!-- Block Button -->
        <form method="POST" onsubmit="return confirm('Are you sure you want to block this user?');" style="display: inline;">
            <input type="hidden" name="username" value="<?php echo htmlspecialchars($username_filter); ?>">
            <button type="button" onclick="openBlockModal('<?php echo htmlspecialchars($username_filter); ?>')" class="block-btn">Block</button>
        </form>
    <?php else: ?>
        <!-- Activate Button -->
    ...
    <?php endif; ?>
        ...
        <?php else: ?>
            <p>Accountant profile not found.</p>
        <?php endif; ?>
